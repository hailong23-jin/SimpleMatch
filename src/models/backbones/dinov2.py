...
import math
import logging
from typing import Sequence, Tuple, Union, Callable, List, Any, Dict
...
class Attention(nn.Module):
    ...
    def forward(self, x: Tensor, new_attn=False) -> Tensor:
        B, N, C = x.shape
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, new_attn=False) -> Tensor:
        def attn_residual_func(x: Tensor) -> Tensor:
            out = self.attn(self.norm1(x), new_attn)
            return self.ls1(out)
...
class NestedTensorBlock(Block):
    ...
    def forward(self, x_or_x_list, new_attn=False):
        if isinstance(x_or_x_list, Tensor):
            return super().forward(x_or_x_list, new_attn)
...
class DinoVisionTransformer(nn.Module):
    def __init__(
        ...
        self.pos_embed = nn.Parameter(torch.zeros(1, num_patches + self.num_tokens, embed_dim))  # num_patches + self.num_tokens
...
def get_transformer_block(dim, num_heads=12):
    norm_layer = partial(nn.LayerNorm, eps=1e-6)
